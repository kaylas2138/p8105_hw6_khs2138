...
- [Problem 2](#problem-2)
  - [Todo: Update to 5000 and add CI](#todo-update-to-5000-and-add-ci)
- [Problem 3](#problem-3)
...
``` r
fit_1 = lm(bwt ~ gaweeks + blength, data = birth_df)
...
fit_2 = lm(bwt ~ bhead * blength * babysex, data = birth_df)
birth_df |>
...
birth_df |>
  gather_predictions(fit, fit_1, fit_2) |>
...
birth_cv = birth_cv |>
  mutate(
    ...
    model_1 = map(train, \(df) fit_1),
    model_2 = map(train, \(df) fit_2)) |>
  mutate(
...
```
    ...
    ## # A tibble: 100 × 9
    ##    train    test     .id   model  model_1 model_2 rmse_mod rmse_mod1 rmse_mod2
    ##    <list>   <list>   <chr> <list> <list>  <list>     <dbl>     <dbl>     <dbl>
    ##  1 <tibble> <tibble> 001   <lm>   <lm>    <lm>        283.      330.      283.
    ##  2 <tibble> <tibble> 002   <lm>   <lm>    <lm>        297.      330.      294.
    ##  3 <tibble> <tibble> 003   <lm>   <lm>    <lm>        290.      318.      285.
    ##  4 <tibble> <tibble> 004   <lm>   <lm>    <lm>        295.      323.      290.
    ##  5 <tibble> <tibble> 005   <lm>   <lm>    <lm>        285.      321.      282.
    ##  6 <tibble> <tibble> 006   <lm>   <lm>    <lm>        286.      332.      287.
    ##  7 <tibble> <tibble> 007   <lm>   <lm>    <lm>        288.      337.      289.
    ##  8 <tibble> <tibble> 008   <lm>   <lm>    <lm>        296.      336.      294.
    ##  9 <tibble> <tibble> 009   <lm>   <lm>    <lm>        281.      341.      282.
    ## 10 <tibble> <tibble> 010   <lm>   <lm>    <lm>        293.      326.      288.
    ## # ℹ 90 more rows
...
<img src="p8105_hw6_khs2138_files/figure-gfm/unnamed-chunk-3-1.png" width="90%" />
